...
p1 = ggplot(nation_rank_, aes(x=reorder(nation_rank_$Var1, desc(nation_rank_$Freq)), y=nation_rank_$Freq)) + geom_bar(stat = 'identity', aes(fill= nation_rank_$Freq)) + xlab('Nation') + ylab('Number of Artists') + ggtitle('Number of Artists by Nationality') + theme(axis.text.x = element_text(angle = 70, hjust = 1, size = 10))
...
#   to_replace <- paste0(artist_genre$artist[i], ' at the ')
#   artist_genre$museums[i] <-  gsub(to_replace, '', artist$museums[i])
# }
...
# my_index <- list()
# for(i in 1:nrow(artist_genre)) {
#   key = artist_genre$artist[i]
#   vals = unlist(strsplit(artist_genre$museums[i], ", "))
#   create_idx(key, vals)
# }

# Do the same thing to genre
my_index <- list()
for(i in 1:nrow(artist_genre)) {
